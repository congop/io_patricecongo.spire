...
def test_entry_created(host):
    ...
    assert set(["unix:user:etcd", "unix:gid:1000"]) == set( entry.get("selector") or [] )
    assert [ "node1.local1", "node1.local2"] == entry.get("dns_name"), f"outcome={outcome}"
